...
select no, t1.noBorrowers, t2.avgBorrowers
from (select no, count(*) as numBorrowers, type
    from customer
    join borrower on cname = name
    join loan on lno = no
    group by no) t1, (select type, avg(q1.noBorrowers) as avgBorrowers
                    from (select no, count(*) as numBorrowers, type
                    from customer
                    join borrower on cname = name
                    join loan on lno = no
                    group by no) q1
                    group by type) t2
where t1.type = t2.type and t1.noBorrowers > t2.avgBorrowers;
